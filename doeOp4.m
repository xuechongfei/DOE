...
seta = 0.1;
for m = 1:200
    F_image = fftshift(fft2(estimated_image));%第k次迭代时，输出光场
    ...
    F_estimated_image =(idea_image_bw.*absF_image*seta + idea_image).*exp(1i*F_image_phase);%对输出光场振幅和理论输出光场振幅进行交换
    IF_image = ifft2(ifftshift(F_estimated_image));%反傅里叶变换后，计算的k+1次输入光场
