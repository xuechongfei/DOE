...
for m = 1:200
    ...
    leta = exp(-200*((P1 - P2)*sum(sum(absF_image))).^2);
    leta1 = 0;
    F_estimated_image = ((1- leta1).*idea_image + idea_image_bw.*absF_image.*leta + (1 - idea_image_bw).*absF_image.*leta1).*exp(1i*F_image_phase);%对输出光场振幅和理论输出光场振幅进行交换
    IF_image = ifft2(F_estimated_image);%反傅里叶变换后，计算的k+1次输入光场
    IF_image_phase = angle(IF_image);%保留相位
    IF_image_phase = floor(mod(IF_image_phase, 2*pi)/(2*pi/16)) * (2*pi/16);
...
subplot(1,3,3)
imshow(IF_image_phase/max(max(IF_image_phase)))
set(gca,'fontname','times new roman','fontsize',16);    %设置图形对象属性
