...
clc;
clear;
gray = im2double(255 - rgb2gray(imread('1.jpg')));            %读入初始图,作为初始迭代分布振幅 
Amplitude = imresize(gray,[512,512],'bilinear');             %适应调制器分辨率，可注释掉
...
%  GS算法
   ...
   G0_GSNew = 1*G0_Fie./abs(G0_GS);          %取相位值,频域作全1幅值约束，相位全息图
